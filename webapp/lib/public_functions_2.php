...
function select_testdesc($sqlqry){
	$_SESSION['tcstest_host_db'] = "172.16.0.36:1521/tcstest";
	$testconnect_db = oci_connect($_SESSION['tcstest_username'], $_SESSION['tcstest_password'], $_SESSION['tcstest_host_db'], 'AL32UTF8');
	$testparseqry = oci_parse($testconnect_db,$sqlqry); // Parse the connection and DB.
	oci_execute($testparseqry, OCI_DESCRIBE_ONLY); // Execute the query
	$ncols = oci_num_fields($testparseqry);
	
	for ($i = 1; $i <= $ncols; $i++) {
		$column_name['FIELD'][$i] = oci_field_name($testparseqry, $i);
		$column_name['TYPE'][$i]  = oci_field_type($testparseqry, $i);
    	$column_name['SIZE'][$i]  = oci_field_size($testparseqry, $i);

	}
	return $column_name;
}

function select_column($sqlqry){
	$connect_db = oci_connect($_SESSION['username1'], $_SESSION['password1'], $_SESSION['host_db1'], 'AL32UTF8');
$parseqry = oci_parse($connect_db,$sqlqry); // Parse the connection and DB.
oci_execute($parseqry, OCI_DESCRIBE_ONLY); // Execute the query
$ncols = oci_num_fields($parseqry);

	for ($i = 1; $i <= $ncols; $i++) {
		$column_name[] = oci_field_name($parseqry, $i);
	}
	return $column_name;
}

function select_testcolumn($sqlqry){
	$_SESSION['tcstest_host_db'] = "172.16.0.36:1521/tcstest";
	$testconnect_db = oci_connect($_SESSION['tcstest_username'], $_SESSION['tcstest_password'], $_SESSION['tcstest_host_db'], 'AL32UTF8');
	$testparseqry = oci_parse($testconnect_db,$sqlqry); // Parse the connection and DB.
	oci_execute($testparseqry, OCI_DESCRIBE_ONLY); // Execute the query
	...
	for ($i = 1; $i <= $ncols; $i++) {
		$column_name[] = oci_field_name($testparseqry, $i);
	}
	return $column_name;
}

function select_reqcolumn($sqlqry){
$ksreqconnect_crmdb = oci_connect($_SESSION['ksrequsername2'], $_SESSION['ksreqpassword2'], $_SESSION['ksreqhost_db2'], 'AL32UTF8');
$parseqryksreq = oci_parse($ksreqconnect_crmdb, $sqlqry); // Parse the connection and DB.
oci_execute($parseqryksreq, OCI_DESCRIBE_ONLY); // Execute the query
$ncols = oci_num_fields($parseqryksreq);

	for ($i = 1; $i <= $ncols; $i++) {
		$column_name[] = oci_field_name($parseqryksreq, $i);
	}
	return $column_name;
}
...
function select_ksreq($sqlqry_selectksreq)
{
	//include("config.php");
	...
	try {
		...
		if(oci_execute($parseqryksreq))
		{
			...
			return oci_error($parseqryksreq); 	
		}
	} 
	catch(Exception $e) {
		echo 'Message: ' .$e->getMessage();
